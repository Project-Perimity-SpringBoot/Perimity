 ...
 *
 * Byte-identical to auth-service's filter apart from the package. Silent on
 * failure by design: a bad token leaves the context empty and Spring Security
 * answers 401 from its own entry point. Writing an error here would produce two
 * different 401 bodies for the same situation.
 *
