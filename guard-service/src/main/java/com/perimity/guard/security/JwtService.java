 ...
 *
 * This is deliberately not a copy of Omkar's JwtService - his has issue(), and
 * a service that can mint tokens can mint one for any role. Five services that
 * only ever read means one place to audit if a token is ever forged.
 *
 * Verification is local: the signature is checked against the shared secret with
 * no network call, so a scan at the gate does not wait on auth-service and a
 * restart of auth-service does not stop the gate.
 *
 * The secret has NO default. If JWT_SECRET is missing the service refuses to
 * start, and that is correct: a service that falls back to a known string will
 * happily accept forged tokens signed with the string that is in Git.
 *
 * That means you must set JWT_SECRET before running from STS. Run Configurations
 * -> your app -> Environment -> New. Same value as everyone else.
 */
@Service
